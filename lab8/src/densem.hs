...
sem Pdiv (VI n2 : VI n1 : s) | n2 /= 0 = VI (n1 `mod` n2) : VI (n1 `div` n2) : s
...
instance Show P where
  showsPrec _ (Pint n) = showsPrec 0 n
  ...
  showsPrec _ (Pcond p1 p2) =
    ("cond [" ++) . showsPrec 0 p1 . (" | " ++) . showsPrec 0 p2 . ("]" ++)
  showsPrec _ (Ploop p) = ("loop [" ++) . showsPrec 0 p . ("]" ++)
...
instance Read P where
  readsPrec d s =
    readParen False (\s' -> [(Pint n, r) | (n, r) <- readsPrec 0 s']) s
      ...
      ++ readParen (d > 0) (\s' -> [(Pseq p1 p2, r) | (p1, t) <- readsPrec 1 s', (p2, r) <- readsPrec 0 t]) s
      ++ readParen False (\s' -> [(Pcond p1 p2, r) | ("cond", t1) <- lex s', ("[", t2) <- lex t1, (p1, t3) <- readsPrec 0 t2, ("|", t4) <- lex t3, (p2, t5) <- readsPrec 0 t4, ("]", r) <- lex t5]) s
      ++ readParen False (\s' -> [(Ploop p, r) | ("loop", t1) <- lex s', ("[", t2) <- lex t1, (p, t3) <- readsPrec 0 t2, ("]", r) <- lex t3]) s
